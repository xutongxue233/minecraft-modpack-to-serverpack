...
项目处于 MVP 早期阶段。当前重点已经覆盖输入解析、平台元数据补全、下载校验、报告生成和桌面工作流。完整服务端目录输出、加载器服务端安装脚本、overrides 合并规则和发布自动化会继续完善。
...
- 生成最终服务端包目录和筛选后的 `mods/`。
- 按规则合并 `overrides/` 和 `server-overrides/`。
- 支持 Fabric、Quilt、Forge、NeoForge 的服务端加载器安装。
