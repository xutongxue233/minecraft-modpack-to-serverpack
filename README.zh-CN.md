...
- Mod 文件下载会按 PCL 思路优先尝试 MCIM 镜像，再回退 CurseForge/Modrinth 官方 CDN；BMCLAPI 用于基础核心和加载器类资源。
- 生成服务端包目录，包含筛选后的 `mods/`、服务端安全 overrides、`server-core.json`、EULA 占位文件、JVM 参数、安装脚本和启动脚本。
- 对服务端安全的 `overrides/` 文件做原样复制，不解析配置 JSON，兼容旧版 Mod 的非标准配置文件。
...
- 支持在桌面端配置 Java Home；直接安装核心时也会参考 PCL 的候选路径和关键词递归搜索本机 Java，并按核心版本自动选择兼容版本。
- 生成的服务端脚本会先读取 `java-home.txt`，再回退到 `JAVA_HOME` 和系统 `java`。
- 下载阶段按 Mod 下载和核心下载分组显示进度；Mod 下载展示已完成数量、百分比和当前文件，核心下载展示字节进度。

## 当前状态

项目处于 MVP 早期阶段。当前已经覆盖输入解析、平台元数据补全、下载校验、服务端 Mod 自动筛选、远程项目级规则库、初版服务端目录生成、可选服务端核心直接安装、可选优化启动脚本、可选 zip 输出、报告生成和桌面工作流。

兼容性会通过真实整合包 smoke test 和自动化集成测试逐步扩展。下方列出的 Minecraft 版本表示仓库中已验证过对应工作流，不代表该版本的所有整合包都能无需规则更新直接运行。
...
| Minecraft | 加载器 / 核心 | 测试整合包或场景 | 已验证范围 | 状态 |
| --- | --- | --- | --- | --- |
| 1.20.1 | Forge 47.4.10 | CurseForge zip，TerraFirmaFarHorizons v1.3.1 | Manifest 解析、CurseForge 元数据补全、Mod/核心下载、客户端 Mod 过滤、报告生成、服务端脚本、优化脚本流程 | 已测试 |
| 1.20.1 | Fabric 0.15.x | Modrinth `.mrpack` 集成测试夹具 | 带 hash 校验的 Mod 下载、按 env 过滤、报告生成、zip 输出 | 自动化集成测试 |
| 1.20.1 | Vanilla | 生成式集成测试夹具 | 直接下载 `server.jar`、生成启动脚本、启动测试到 EULA 检查 | 自动化集成测试 |
| 1.12.2 | Forge 14.23.5.2860 | CurseForge zip，RLCraft v2.9.3 | Manifest 解析、`overrides/` 原样复制、旧版非标准 `mcmod.info` 容错、旧版 Forge 安装产物识别、优化脚本启动到 EULA 检查 | 启动 smoke 已通过；完整运行仍取决于 Mod 下载和规则覆盖 |

新增兼容性声明时，优先使用真实整合包 smoke test 和生成的 `conversion-report.json` 作为依据。如果只完成了解析但还没有跑到启动脚本阶段，应标记为部分通过。

## Overrides 处理策略

`overrides/` 和 `server-overrides/` 会被当作文件树处理，而不是当作 JSON 或 TOML 等结构化配置解析。转换器会对服务端安全文件做逐字节原样复制，因为很多旧版 Minecraft Mod 配置虽然使用 `.json` 后缀，但内容可能包含注释、未转义换行或其他非标准语法。

转换器只会过滤明显属于客户端本地状态的路径，例如 `options.txt`、`shaderpacks/`、`resourcepacks/`、截图、存档、日志和 `overrides/mods/`。Mod 文件应来自整合包清单和平台元数据，这样报告才能保留 hash、下载状态和筛选决策。
...
packages/*/test/     包级测试目录，与生产 src 代码分离
docs/                需求文档、架构文档、开发计划
```
...
- 扩展旧版 Forge 1.12 带完整 Mod 下载集的运行 smoke test。
- 强化加载器核心直接安装能力，增加更多兼容性校验和失败回退。
- 优化远程客户端 Mod 规则库的诊断和贡献流程。
- 增加 GitHub Release 自动化，发布 Windows 安装包和便携版。
- 改进 packwiz 远程元数据和 hash 处理。

## 参与贡献

欢迎提交 issue 和 pull request。提交 PR 前请阅读 [CONTRIBUTING.md](./CONTRIBUTING.md) 和 [提交信息规范](./docs/commit-convention.md)。

Bug、转换失败、功能建议、客户端 Mod 规则更新请优先使用 GitHub issue 模板。反馈问题时建议提供：
