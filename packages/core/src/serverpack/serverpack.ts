...
function renderStartBatch(): string {
  return [
    ...
    "  \"%JAVA_CMD%\" @user_jvm_args.txt -jar fabric-server-launch.jar nogui",
    "  exit /b %ERRORLEVEL%",
    ")",
    "if exist quilt-server-launch.jar (",
    "  \"%JAVA_CMD%\" @user_jvm_args.txt -jar quilt-server-launch.jar nogui",
    "  exit /b %ERRORLEVEL%",
    ")",
    "if exist server.jar (",
    "  \"%JAVA_CMD%\" @user_jvm_args.txt -jar server.jar nogui",
    "  exit /b %ERRORLEVEL%",
    ")",
